...
Use the `sofa_magic_wrapper()` function when you use a raw data file from the Acutelines database [@Avest2021]. SOFA related variables are coded as `SOFA_[timespan]h_[variable]`. Make sure all variables are present.
...
set to `TRUE` the function will return all the inidivual elements of the SOFA score instead of only the totals.

``` R
result <- sofa_magic_wrapper(df, interval=24, timespan=72)
```

By default, the function only returns the total SOFA score, but you can also return the individual components of the SOFA score by setting `return_df = TRUE`. This will return a dataframe with all the individual components of the SOFA score.
...
# Calculations
The SOFA (Sequential Organ Failure Assessment) [@Vincent1996] calculation functions are designed to provide the worst SOFA score for a patient within a specific interval.
...
Many respiratory parameters tend to be missing. The `sofa_respiration()` function calculates the respiratory points of the SOFA score
...
4. Unrealistic FiO~2~ for the corresponding oxygen delivery device are removed:
   + If oxygen delivery device is 'nasal cannula' and FiO~2~ > 44%, it is treated as missing.
   + If oxygen delivery device is 'venturi mask' and FiO~2~ > 60%, it is treated as missing.
   + If oxygen delivery device is 'non-rebreather' and FiO~2~ > 90%, it is treated as missing.
5. Fractional FiO~2~ Values:  
...
7. Estimation from type of O~2~ delivery and flow rate:
...
dosages need to be provided as µg/kg/min.
...
## Renal
Calculates the SOFA score component for the renal function using the `sofa_renal()` function. Points are assigned based on the creatinine level (µmol/L).
This section does not correct for unrealistic creatinine levels, as this data is generally already curated by the lab.

The following table lists the assigned points:
...
Urine output < 500 mL/day correlates with moderate renal dysfunction, while < 200 mL/day indicates severe dysfunction. Urine output is currently not implemented in the calculation, as this data is often missing or registered inadequately.
